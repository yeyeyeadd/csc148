...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        for req in workout_class.get_required_certificates():
            if req not in self._certificates:
                return False
        return True
...
class Gym:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        if not self._instructors[instr_id].can_teach(
            ...
            return False
        elif time_point not in self._schedule:
            self._schedule[time_point] = {}
            self._schedule[time_point][room_name] = \
                (self._instructors[instr_id], self._workouts[workout_name], [])
            return True
        elif room_name not in self._schedule[time_point]:
            for teacher in self._schedule[time_point]:
                if self._schedule[time_point][teacher][0] == \
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for rom in self._schedule[time_point]:
            if client in self._schedule[time_point][rom][2]:
                return False
        for dat in self._schedule:
            for i in self._schedule[dat]:
                if dat == time_point and workout_name == \
                        self._schedule[dat][i][1].get_name() and \
                        self._rooms[i] > len(self._schedule[dat][i][2]):
                    self._schedule[time_point][i][2].append(client)
                    return True
        return False
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        if time_point not in self._schedule:
            ...
            for rom in self._schedule[time_point]:
                lis.append((self._schedule[time_point][rom][0].name,
                            self._schedule[time_point][rom][1].get_name(), rom))
        return lis
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        for i in range(len(lis)):
            if len(lis) == 1:
                ...
                if lis[i] < time1 > lis[i + 1]:
                    sta = i + 1
                elif lis[i] == time1:
                    sta = i
                if lis[i] >= time2 >= lis[i - 1]:
                    end = i + 1
        for i in lis[sta: end]:
            for rom in self._schedule[i]:
                ans[self._schedule[i][rom][0].get_id()] += 1
        return ans
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        ans = []
        hour = self.instructor_hours(time1, time2)
        for i in hour:
            ans.append((i, self._instructors[i].name, hour[i],
                        (self._instructors[i].get_num_certificates() *
                         BONUS_RATE + base_rate) * hour[i]))
        return ans
