...
class Block:
    ...
    def smash(self) -> bool:
        ...
        if self.smashable():
            ...
            for i in lst:
                self.children.append(Block(i, self._child_size(),
    ...
    def combine(self) -> bool:
        ...
        if len(self.children) == 4 and self.level == self.max_depth - 1:
            ...
            count = []
            for child in self.children:
                if child.colour not in lst:
                    lst.append(child.colour)
                    count.append(1)
                else:
                    count[lst.index(child.colour)] += 1
            if len(lst) == 3:
                mj = lst[count.index(max(count))]
                self.colour = mj
                self.children = []
                return True
        return False
        ...
        if len(self.children) == 0:
            ...
            for child in self.children:
                if len(child.children) == 0:
                    new.children.append(
                        Block(child.position, child.size, child.colour,
                              child.level, child.max_depth))
                else:
