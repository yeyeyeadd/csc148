...
class Student:
    ...
    === Private Attributes ===
    _ans: the dict content each Question point to it's Answer
    ...
    def has_answer(self, question: Question) -> bool:
        ...
        """
        # TODO: complete the body of this method
        if question in self._ans and \
                question.validate_answer(self._ans[question]):
            return True
        else:
            return False
...
class Course:
    ...
    def all_answered(self, survey: Survey) -> bool:
        ...
        for student in self.students:
            for que in survey.get_questions():
                if not student.has_answer(que):
                    return False
        return True
